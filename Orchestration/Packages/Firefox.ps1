...
$OldVersions = Get-ChildItem -Path $PackagePath -Filter Mozilla_Firefox_*
...
`$NewPackageName = "Mozilla_Firefox_$($CurrentVersion)_APPV_Open_USR"
...
$FirefoxQueue | Out-File -FilePath \\usc.internal\usc\appdev\General\Packaging\PackageQueue\Firefox.ps1 -Force
$PackageVM = & "$Working\PackageOrchestrator.ps1" -Build -PackageName "FireFox"
Do {
    Write-Verbose "Waiting for package to be built..."
    Start-Sleep -Seconds 20
} While (Test-Path -Path \\usc.internal\usc\appdev\General\Packaging\PackageQueue\Firefox.ps1)
    Write-Verbose "Package complete. Destroying VM"
    #& "$Working\PackageOrchestrator.ps1" -Destroy -PackageVM $PackageVM

} else {
    Write-Verbose "Newest Firefox already packaged: $($NewestPackage.Version)"
}
